...
export default function DirectMessagesDrawer(props: DirectMessagesDrawerProperties): React.JSX.Element {
  ...
  const renderConversations = useCallback(() => {
    ...
    return conversations.map((convo) => <MemberMemo isButton={true} memberName={convo.userName} isOnline={convo.isActive || false} />);
  }, [conversations]);
  ...
  return (
    <div className="card" style={isShown ? { ...styles.drawer, ...styles.open } : styles.drawer}>
      ...
      <div className="card-body" style={styles.body}>
        <ul className="list-group list-group-flush">{renderConversations()}</ul>
      </div>
      <div className="card-footer">
        <div className="row">
          <div className="col-4 d-flex p-1">
            <button className="btn btn-success shadow flex-grow-1" type="button" title="New">
