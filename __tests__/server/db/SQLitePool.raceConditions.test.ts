...
describe("SQLitePool Race Conditions", () => {
  ...
  beforeEach(() => {
    pool = new SQLitePool(":memory:", 2); // A pool with a maximum of 2 connections.
  });
  ...
  test("should handle concurrent connection requests and maintain consistency", async () => {
    ...
    expect(await pool.pendingRequestsSize).toBe(0); // No pending requests
  });
  ...
  test("should ensure pending requests are fulfilled in order", async () => {
    ...
    expect(await pool.pendingRequestsSize).toBe(3); // There should be 3 pending requests
    ...
    expect(await pool.pendingRequestsSize).toBe(0);
  });
  ...
  test("should handle high contention with long-running tasks", async () => {
    ...
    expect(await pool.pendingRequestsSize).toBe(0); // No pending requests left
  });
  ...
  //TODO: RESOLVE THIS FAILING TEST!
  test("should recover from a failed connection", async () => {
    const simulateTask = async (taskId: number) => {
      ...
      if (taskId === 3) {
        ...
        await pool.closeAll(); // Simulate database shutdown
    ...
    expect(await pool.size).toBe(2); // Recovered back to max size
    expect(await pool.pendingRequestsSize).toBe(0);
  });
});
