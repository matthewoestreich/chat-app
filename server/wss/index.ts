...
export default function createWebSocketApp<T extends sqlite3.Database>(databasePool: DatabasePool<T>) {
  ...
  wsapp.on("CONNECTION_ESTABLISHED", async (client, { request }) => {
    ...
    const { db, release } = await databasePool.getConnection();
  ...
  async function listenAsync(options: ServerOptions): Promise<void> {
